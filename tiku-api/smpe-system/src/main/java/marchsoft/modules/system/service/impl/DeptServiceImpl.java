...
//                        throw new BadRequestException("您没有查询部门目标dept：" + criteria.getPid() + "的权限。");
    ...
    @Override
    public List<Dept> queryAllTop() {

        return deptMapper.selectList(new LambdaQueryWrapper<Dept>().eq(Dept::getPid,0));
    }

    /**
     * 功能描述: 通过isTop判断是否查询顶级部门
     *
     * @param isTop isTop
     * @return List
     * @author mfei
     ...
     */
    @Override
    public List<Dept> queryAllDept(boolean isTop) {
//        LambdaQueryWrapper<Dept> queryWrapper = new LambdaQueryWrapper<>();
//        if (isTop) {
//            queryWrapper.eq(Dept::getPid,0);
//        }
//        deptMapper.selectList(queryWrapper);
        return deptMapper.queryAllDept(isTop);
    }
