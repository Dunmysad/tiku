...
public class StudentListener implements ReadListener<StudentExcel> {
     ...
     */
    private IEStudentService ieStudentService;
    /**
     * 如果使用了spring,请使用这个构造方法。每次创建Listener的时候需要把spring管理的类传进来
     *
     * @param ieStudentService
     */
    public StudentListener(IEStudentService ieStudentService) {
        this.ieStudentService = ieStudentService;
    }
    /**
     * 这个每一条数据解析都会来调用
     *
    ...
    private void saveData() {
        ...
        ieStudentService.addExcel(cachedDataList);
        log.info("存储数据库成功！");
    }
}